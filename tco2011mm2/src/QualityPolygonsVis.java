...
class Pnt {
    ...
    public Pnt sub(Pnt other) {
        return new Pnt(x - other.x, y - other.y);
    }

    public Pnt add(Pnt other) {
        return new Pnt(x + other.x, y + other.y);
    }
...
public class QualityPolygonsVis {
    ...
    void generate(String seed) {
      try {
        ...
        System.out.print("sidesDiff = " + sidesDiff + ", ");
    ...
    String validatePoly(int[] poly, int n) {
        ...
        if (100*100 * minR2 < maxR2 * sq(100-radiiDiff)) {
            ...
            return "" + sumX + " " + sumY + " " + minR2 + " " + maxR2 + " polygon must be approximately inscribed; here minR = "+minLen+", maxR = "+maxLen+", diff = "+(maxLen-minLen)/maxLen+".";
        }
...
//        os.flush();
        QualityPolygons qp = new QualityPolygons(); 
        return qp.choosePolygons(points, sidesDiff, radiiDiff);
...
//        return ret;
    ...
    public class Vis extends JPanel implements MouseListener, WindowListener {
        ...
        private void showVert(Graphics2D g2) {
            Pnt p0 = p[Pcur[0]];
            Pnt p1 = p[Pcur[1]];
            Pnt p2 = p[Pcur[2]];
            ...
            int cross = G2D.cross(v1, v2);
            println("cross " + cross);
            double cos = cross / (Math.sqrt(G2D.norm2(v1)) * Math.sqrt(G2D.norm2(v2))); 
            println("cos fi " + cos);
            int dot = G2D.dot(v1, v2);
            println("dot " + dot);
            double sin = dot / (Math.sqrt(G2D.norm2(v1)) * Math.sqrt(G2D.norm2(v2))); 
            println("sin fi " + sin);
            ...
            Pnt p4 = p2;
//            p2.x += Math.cos(PENT_ANGLE) * 
            
            long len = (long) Math.sqrt(Math.max(G2D.dist2(p0, p1), G2D.dist2(p1, p2)));
            int dx = (int) Math.abs(len * (sidesDiff / 100.0));
            int dy = (int) Math.abs(len * (sidesDiff / 100.0));
            
            System.out.println("center vert " + p3);
            int left = p3.x - dx;
            int right = p3.x + dx;
            int top = p3.y - dy;
            int bottom = p3.y + dy;
            System.out.println("area vert " + left + "," + top + "," + right + "," + bottom);
            ...
            int centerx = (int) (sumX / 4);
            int centery = (int) (sumY / 4);

            
            sumX = sumX - p3.x + p[Pcur[3]].x;
            sumY = sumY - p3.y + p[Pcur[3]].y;
            int centerx2 = (int) (sumX / 4);
            int centery2 = (int) (sumY / 4);
            int dist0 = (int) Math.sqrt(G2D.dist2(p3, new Pnt(centerx, centery)));
            g2.setColor(Color.LIGHT_GRAY);
            //g2.fillOval(centerx - dist0 / 2, SZ-1 - (centery - dist0 / 2), dist0, dist0);
            g2.fillArc(centerx-dist0, SZ-1-(centery+dist0), dist0*2, dist0*2, 0, 360);

            g2.setColor(Color.ORANGE);
            g2.fillOval(centerx-2, SZ-1-centery-2,5,5);

            g2.setColor(Color.RED);
            g2.fillOval(centerx2-2, SZ-1-centery2-2,5,5);
            
            g2.setColor(Color.YELLOW);
            g2.fillRect(p3.x - dx, SZ-1- (p3.y + dy), dx*2, dy*2);

            g2.setColor(Color.MAGENTA);
            g2.fillOval(p3.x-2, SZ-1-p3.y-2,5,5);
