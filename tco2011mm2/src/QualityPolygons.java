...
//    private int[] linesSorted2;
    ...
    private static final double PENT_ANGLE = Math.PI * 108.0 / 180.0;
...
//        println("dif " + ((totalMemory - freeMemory) / 1024 / 1024) + "mb");
    ...
    private void findPoly(int edges) {
        ...
        if (edges==0 || numPoints < 500 || globalI > numPoints / 2)
            globalI = 0;
...
/*                if (numPoints > 3000) {
                ...
                poly[0] = j;
                int dx1 = vecx[j*numPoints + globalI];
                int dy1 = vecy[j*numPoints + globalI];
...
//        println("convex1 " + convex1 +", convex2 " + convex2 + ", center " + center);
    ...
    private boolean findEdge(long minLen, long maxLen, int edges) {
        ...
        int min3x = 0, max3x = 0, min3y = 0, max3y = 0;
        int centerX = 0, centerY = 0;
        long minDist = 0, maxDist = 0;
        if (N+edges == 4 || N+edges == 5) {
            int p0 = poly[0];
            int p1 = poly[1];
            int p2 = poly[2];
            ...
            if (N+edges==4) {
                p3x = points[p1*2] + v3x;
                p3y = points[p1*2+1] + v3y;
            } else if (N+edges == 5) {
                if (N==3) {
                    //p3x = points[p2*2] + v3x;
                    //p3y = points[p2*2+1] + v3y;
                    p3x = (int) (points[p2*2] * Math.cos(PENT_ANGLE));
                    p3y = (int) (points[p2*2+1] * Math.sin(PENT_ANGLE));
                } else { /* N = 4 */
                    int p3 = poly[3];
                    p3x = (int) (points[p3*2] * Math.cos(PENT_ANGLE));
                    p3y = (int) (points[p3*2+1] * Math.sin(PENT_ANGLE));
                }
            }
            ...
            min3x = Math.max(p3x - dx, 0);
            max3x = Math.min(p3x + dx, MAX_WIDTH);
            min3y = Math.max(p3y - dy, 0);
            max3y = Math.min(p3y + dy, MAX_HEIGHT);
            
            if (numPoints > 2500 && N+edges==4) {
        ...
        nextp: for (int ii = last + 1; ii < numPoints; ii++) {
            ...
            if (/*numPoints > 2000 && */(N+edges == 4 || N+edges == 5) && (newx < min3x || newx > max3x || newy < min3y || newy > max3y)) {
                continue;
            }
...
//                    continue;
    ...
    private boolean validate() {
        ...
        return checkInscribed() && isConvex() /*&& checkLen()*/;
    }
