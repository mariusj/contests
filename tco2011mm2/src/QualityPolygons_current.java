...
public class QualityPolygons_current {
...
/*                if (numPoints > 3000) {
                ...
                for (int kk = jj + 1; kk < numPoints; kk++) {
                    ...
                    if (edges == 0) {
                        ...
                        if (!checkAngles(3+edges, cross0, norm1, norm2, true))
                            continue;
...
//        println("convex1 " + convex1 +", convex2 " + convex2 + ", center " + center);
    ...
    private boolean checkAngles(int edges, long cross, double norm1, double norm2, boolean first) {
...
//                    continue;
            ...
            double norm1 = Math.sqrt(norm2(dx1, dy1));
            boolean angle = false;
            if (!checkAngles(N + edges, cross, norm1, norm2, false)) {
                angle = true;
                //continue;
            }
            ...
            if (edges == 1) {
                if (isValid(newminLen, newmaxLen)) {
                    if (angle) {
                        print("valid flagged");
                        checkAngles(N + edges, cross, norm1, norm2, false);
                    }
                    found();
                    return true;
                }
